...
export function My4418({management=false}:{management?:boolean}){
 ...
 return <section className="my-dashboard" aria-labelledby="my-heading"><div className="section-heading my-welcome"><div><h1 id="my-heading">{management?'Announcements':'My 4418'}</h1>{!management&&data?<><h2>{greeting}, {data.name.split(' ')[0]}.</h2><p>Here’s what’s happening with the team.</p></>:<p>{management?'Updates from team leadership.':'Your meetings, requests, and team updates.'}</p>}</div>{!management&&state==='ready'&&<button onClick={()=>setRefresh(x=>x+1)}>Refresh</button>}</div>
 ...
 <section className="my-announcement-section" aria-labelledby="announcements-heading"><h2 id="announcements-heading">Announcements</h2><div className="my-announcements">{activeAnnouncements.map(a=><article className={`my-card announcement ${a.severity}`} key={a.id}>{a.severity!=='normal'&&<span className="my-pill">{label(a.severity)}</span>}<h3>{a.title}</h3><p>{a.body}</p><AnnouncementImage path={a.image_path}/><small>{date(a.created_at)}</small></article>)}</div>{!activeAnnouncements.length&&<p className="my-quiet">You’re up to date. No new announcements.</p>}</section>
 <section aria-labelledby="activity-heading"><h2 id="activity-heading">My activity</h2><div className="my-grid"><Card title="My attendance" href="#attendance" action="View attendance">{data.personal.percent===null?<p>No completed required meetings yet.</p>:<p className="my-number">{data.personal.percent}% <small>attendance</small></p>}{data.personal.strikes>0&&<p>{data.personal.strikes} active {data.personal.strikes===1?'strike':'strikes'}</p>}</Card><Card title="My purchase orders" href="https://finance.frc4418.org/" action="View purchase orders">{data.orders.length?<ul className="my-rows">{data.orders.map(p=><li key={p.id}><a href={`https://finance.frc4418.org/#po/${p.id}`}>PO #{p.po_number} · {p.vendor}</a><span>{money(p.amount)} · {label(p.status)}</span>{p.status==='awaiting_approval'&&<small>{p.approvals}/2 approvals for this submission</small>}</li>)}</ul>:<p>No purchase orders yet.</p>}</Card></div></section>
 {data.attention&&(data.robot||data.inventory&&(data.inventory.low>0||data.inventory.out>0))&&<section className="my-team-status" aria-labelledby="status-heading"><h2 id="status-heading">Team status</h2>{data.robot&&<a href="https://pit.frc4418.org/">{data.robot.event}: {data.robot.readiness==='READY'?'Robot ready':data.robot.readiness==='NOT READY'?'Robot needs attention':'Check robot readiness'} · {data.robot.open} open issues →</a>}{data.inventory&&(data.inventory.low>0||data.inventory.out>0)&&<a href="https://inventory.frc4418.org/#restock">{data.inventory.low} low-stock · {data.inventory.out} out-of-stock items →</a>}</section>}
 {data.admin&&<nav className="my-admin" aria-labelledby="admin-heading"><h2 id="admin-heading">Administration</h2><a href="#team-management">Team Management →</a><a href="#announcements">Manage announcements →</a></nav>}
 </>)}
 {!management&&<>
 <nav className="my-quick" aria-labelledby="quick-heading"><h2 id="quick-heading">Quick access</h2><div>{systems.map(link=><a key={link.id} href={link.url!}>{link.name}<span aria-hidden="true"> →</span></a>)}</div></nav>
 <nav className="my-tools" aria-labelledby="tools-heading"><h2 id="tools-heading">Team tools</h2><div>{resources.map(link=>link.url?<a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer">{link.name} ↗</a>:<span key={link.id}>{link.name}<small>Not set up</small></span>)}</div></nav>
 </>}
 </section>;
}
