...
function MemberDialog({children,busy,onClose}:{children:ReactNode;busy:boolean;onClose:()=>void}) {
 const ref=useRef<HTMLDialogElement>(null);
 useLayoutEffect(()=>{const d=ref.current!;const opener=document.activeElement as HTMLElement|null;d.showModal();return()=>{d.close();opener?.focus();};},[]);
 return <dialog className="team-dialog" ref={ref} aria-label="Member editor" onCancel={e=>{e.preventDefault();if(!busy)onClose();}}>{children}</dialog>;
}
export function TeamManagement({workspace}:{workspace:boolean}) {
 ...
 {tab==='Members'&&<>
 <div className="team-view-heading"><h2>Members <span className="team-count">{data.members.length}</span></h2><span>Roles, registration, and team areas</span></div>
 ...
 {!data.members.length&&<p className="team-empty">No team accounts yet. Ask a team administrator to send invitations.</p>}
 {data.members.length>0&&search&&!data.members.some(m=>`${m.display_name} ${m.role} ${data.assignments.filter(a=>a.user_id===m.id&&!a.revoked_at).map(a=>data.positions.find(p=>p.key===a.position_key)?.name).join(' ')}`.toLowerCase().includes(search.toLowerCase()))&&<div className="team-empty"><p>No members match “{search}”.</p><button onClick={()=>setSearch('')}>Clear search</button></div>}
 </>}
 {tab==='Positions'&&<><div className="team-view-heading"><h2>Position ownership</h2><span>Assignments are separate from account roles</span></div><div className="team-position-grid">{data.positions.map(p=><article className="team-member" key={p.key}><h2>{p.name}</h2>{!p.active&&<p>Inactive position</p>}{data.assignments.filter(a=>a.position_key===p.key&&!a.revoked_at).map(a=><button key={a.id} onClick={()=>setSelected(a.user_id)}>{data.members.find(m=>m.id===a.user_id)?.display_name||'Team member'} →</button>)}{!data.assignments.some(a=>a.position_key===p.key&&!a.revoked_at)&&<><p className="team-vacancy">Unassigned</p><p>Choose a member to assign this position.</p><button onClick={()=>setTab('Members')}>Find a member</button></>}</article>)}</div></>}
 ...
 {tab==='Areas'&&<><div className="team-view-heading"><h2>Team areas</h2><span>Shared across the suite</span></div><div className="team-area-list">{data.areas.map(a=><div key={a.id}><strong>{a.name}</strong><span>{a.active?'Active':'Inactive'}</span></div>)}</div>{!data.areas.length&&<p className="team-empty">No areas yet. Add the team’s first functional area below.</p>}
