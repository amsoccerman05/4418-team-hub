...
test('signed out has useful sign-in state',async({page})=>{await page.goto('/');await expect(page.getByRole('heading',{name:'Sign in to Team Hub'})).toBeVisible();await expect(page.getByRole('button',{name:'Sign in',exact:true})).toBeVisible();});
...
for(const width of [390,1440])test(`shared Hub sign-in and header logout ${width}`,async({page})=>{
 ...
 await page.goto('/'+route);await expect(page.getByRole('heading',{name:'Sign in to Team Hub',exact:true})).toBeVisible();await expect(page.getByLabel('Email',{exact:true})).toBeVisible();await expect(page.locator('.suite-header')).toHaveCount(1);await expect(page.getByRole('button',{name:'Sign out',exact:true})).toHaveCount(0);expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
 }
 await setup(page,true);await page.route('**/auth/v1/logout**',r=>r.fulfill({status:204}));await page.goto('/');await expect(page.locator('.suite-header').getByRole('button',{name:'Sign out',exact:true})).toBeVisible();await page.locator('.suite-header').getByRole('button',{name:'Sign out',exact:true}).click();await expect(page.getByRole('heading',{name:'Sign in to Team Hub',exact:true})).toBeVisible();await expect(page.getByRole('heading',{name:'My 4418',exact:true})).toHaveCount(0);
});
